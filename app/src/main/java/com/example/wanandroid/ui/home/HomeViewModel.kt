...
class HomeViewModel : BaseViewModel() {
    ...
    private val _uiBannerState = MutableStateFlow<UiState<List<Banner>>>(UiState.Loading)
    val uiBannerState: StateFlow<UiState<List<Banner>>> = _uiBannerState
    ...
    private val _collectEvent = MutableSharedFlow<String>()
    ...
    private fun loadArticles(page: Int, isRefresh: Boolean) {
        ...
        launchRequest(_uiArticleState) {
            val response = RetrofitClient.api.getArticles(page)
    ...
    fun toggleCollect(article: Article) {
        viewModelScope.launch {
            val currentList = _uiArticleStateList.value.toMutableList()
            val index = currentList.indexOfFirst { it.id == article.id }
            if (index != -1) {
                val oldValue = article.collect ?: false
                val newValue = !oldValue
                currentList[index] = article.copy(collect = newValue) // 更新列表中的数据
                _uiArticleStateList.value = currentList // 触发 UI 重组
                try {
                    if (newValue) {
                        RetrofitClient.api.collectArticle(article.id!!)
                        _collectEvent.emit("收藏成功")
                    } else {
                        RetrofitClient.api.unCollectArticle(article.id!!)
                        _collectEvent.emit("取消收藏成功")
                    }
                } catch (e: Exception) {
                    // 失败回滚
                    currentList[index] = article.copy(collect = oldValue)
                    _uiArticleStateList.value = currentList
                    _collectEvent.emit("操作失败: ${e.message}")
                }
            }
        }
    }
